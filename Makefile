...
program_name := $(type_of_microcontroller)program
...
map_path := Build_Results/Map/K1921VK01Tprogram.map
...
$(program_name).hex: $(program_name).elf
	...
	@echo hexname$(program_name) #for help searching name of output file at script
...
.PHONY : clean
clean: 
	-rm $(target_path)$(program_name).* $(obj_path)*.o $(obj_path)*.d $(map_path) $(target_path)*.log